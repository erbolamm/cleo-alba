...
class AlbaAdminScreen extends StatefulWidget {
  ...
  const AlbaAdminScreen({super.key, this.onClearHistory});
...
class _AlbaAdminScreenState extends State<AlbaAdminScreen> {
  ...
  Future<void> _stopServerAndClose() async {
    try {
      await http
          .post(Uri.parse('http://127.0.0.1:8080/api/shutdown'))
          .timeout(const Duration(seconds: 4));
    } catch (_) {}
    SystemNavigator.pop();
  }
  ...
  Future<void> _clearHistory() async {
    // Borrar historial del servidor local (si está activo)
    try {
      await http
          .post(Uri.parse('http://127.0.0.1:8080/api/clear'))
          .timeout(const Duration(seconds: 4));
    } catch (_) {
      // Servidor no activo, no importa
    }
    // Borrar historial guardado en la app (SharedPreferences + memoria)
    await widget.onClearHistory?.call();
    _showSnack('🧹 Historial borrado. Cleo empezará de cero');
  }
